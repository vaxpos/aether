...
let
  cfg = config.wayland.windowManager.aether;
...
{
  options = {
    wayland.windowManager.aether = with lib; {
      enable = mkOption {
        type = types.bool;
        default = false;
        description = "Whether to enable aetherwm, a Wayland compositor based on dwl.";
      };
      package = lib.mkOption {
        type = lib.types.package;
        default = self.packages.${pkgs.stdenv.hostPlatform.system}.aether;
      ...
      systemd = {
        enable = mkOption {
          ...
          example = false;
          description = ''
            Whether to enable {file}`aether-session.target` on
      ...
      settings = mkOption {
        type =
          ...
          let
            valueType =
              ...
              // {
                description = "aether configuration value";
              };
          in
          valueType;
        default = { };
        description = ''
          ...
          quoted. See <https://aetherwm.github.io/docs> for more examples.

          ::: {.note}
          This option uses a structured format that is converted to aether's
  ...
  config = lib.mkIf cfg.enable (
    let
      finalConfigText =
        ...
        + lib.optionalString (cfg.autostart_sh != "") "\nexec-once=~/.config/aether/autostart.sh\n";

      validatedConfig = pkgs.runCommand "aether-config.conf" { } ''
        cp ${pkgs.writeText "aether-config.conf" finalConfigText} "$out"
        ${cfg.package}/bin/aether -c "$out" -p || exit 1
      '';
    in
    {
      # Backwards compatibility warning for old string-based config
      warnings = lib.optional (builtins.isString cfg.settings) ''
        wayland.windowManager.aether.settings: Using a string for settings is deprecated.
      ...
      xdg.configFile = {
        ...
        "aether/autostart.sh" = lib.mkIf (cfg.autostart_sh != "") {
          source = autostart_sh;
          executable = true;
        };
      };
      systemd.user.targets.aether-session = lib.mkIf cfg.systemd.enable {
        Unit = {
          Description = "aether compositor session";
