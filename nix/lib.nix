...
let
  ...
  /**
    Convert a structured Nix attribute set into aether's configuration format.

    This function takes a nested attribute set and converts it into aether-compatible
    ...
    `key = value` pairs. Lists are expanded as duplicate keys to match aether's expected format.
    ...
    * `bottomCommandsPrefixes` - A list of prefixes to define **bottom** commands (default: `[]`).
    ...
    - Lists are converted into multiple entries, ensuring compatibility with aether.
    ...
    ```
    toaether :: AttrSet -> AttrSet -> String
    ```

    # Examples
    :::{.example}

    ## Basic aetherwc configuration
