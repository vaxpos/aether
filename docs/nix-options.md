...
description: NixOS and Home Manager configuration options for aetherwm.
---
...
**System-level options via `programs.aether`.**

### enable



Whether to enable aether, a wayland compositor based on dwl\.
...
*Declared by:*
 - [\<aether/nix/nixos-modules\.nix>](https://github.com/aetherwm/aether/blob/main/nix/nixos-modules.nix)
...
Whether to add a login entry to the display manager for aether\. Only has effect if a display manager is configured (e\.g\. SDDM, GDM via ` services.displayManager `)\.
...
**Configure aetherwm declaratively via `wayland.windowManager.aether`.**

### enable



Whether to enable aetherwm, a Wayland compositor based on dwl\.
...
*Declared by:*
 - [\<aether/nix/hm-modules\.nix>](https://github.com/aetherwm/aether/blob/main/nix/hm-modules.nix)
...
Shell script to run on aether startup\. No shebang needed\.

When this option is set, the script will be written to
` ~/.config/aether/autostart.sh ` and an ` exec-once ` line
...
Extra configuration lines to add to ` ~/.config/aether/config.conf `\.
...
aether configuration written in Nix\. Entries with the same key
should be written as lists\. Variables and colors names should be
quoted\. See [https://aetherwm\.github\.io/docs](https://aetherwm\.github\.io/docs) for more examples\.

**Note:** This option uses a structured format that is converted to aether’s
...
Whether to enable ` aether-session.target ` on
aether startup\. This links to
...
[
  ...
  "systemctl --user start aether-session.target"
]
```
