...
> **Warning:** Some graphics cards require setting the `WLR_DRM_NO_ATOMIC` environment variable before aether starts to successfully enable tearing.
...
Or run aether with the environment variable:

```bash
WLR_DRM_NO_ATOMIC=1 aether
```
...
If aether cannot display correctly or shows a black screen, try selecting a specific GPU:

```bash
# Use a single GPU
WLR_DRM_DEVICES=/dev/dri/card1 aether

# Use multiple GPUs
WLR_DRM_DEVICES=/dev/dri/card0:/dev/dri/card1 aether
```
...
> **Warning:** Use a `DISPLAY` value other than `:1` to avoid conflicting with aetherwm.
