...
aetherwm supports two types of scratchpads: the standard pool (Sway-like) and named scratchpads.
...
Named scratchpads are bound to specific keys and applications. When triggered, aetherwm will either launch the app (if not running) or toggle its visibility.
