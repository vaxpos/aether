...
> **Warning:** Some graphics cards require setting `WLR_DRM_NO_ATOMIC=1` before aether starts for tearing to work. Add it to `/etc/environment` and reboot, or launch aether with `WLR_DRM_NO_ATOMIC=1 aether`. See [Monitors — Tearing](/docs/configuration/monitors#tearing-game-mode) for details.
